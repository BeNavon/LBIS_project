function [strikes, offs] = getStrikes(dataArr, varargin)
...
%   above a threshold
    ...
    v_x = gradient(smoothdata(dataArr(:,1), 'movmedian', 50));
    v_y = gradient(smoothdata(dataArr(:,2), 'movmedian', 50));
    v_z = gradient(smoothdata(dataArr(:,3), 'movmedian', 50));
