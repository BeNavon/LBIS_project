...
class GaitPhaseDataset(Dataset):
    def __init__(self, root_dir, sequence_length=128, subjects=None, transform=None):
        ...
        if subjects is not None:
            # Assuming the subject folder is the first folder in the path.
    ...
    def __getitem__(self, idx):
        ...
        # We keep columns 6 to 11 (shank) and 12 to 17 (thigh) → 12 channels total.
        shank = imu_data[:, 6:12]
        thigh = imu_data[:, 12:18]
        imu_selected = np.concatenate([shank, thigh], axis=1)
        ...
        # Extract a random window of fixed length.
        if min_length > self.sequence_length:
            start_idx = random.randint(0, min_length - self.sequence_length)
        else:
            start_idx = 0  # Alternatively, you could pad shorter sequences.
        end_idx = start_idx + self.sequence_length
        imu_window = imu_selected[start_idx:end_idx, :]  # Shape: (sequence_length, 12)
        
        # Use the gait phase (HeelStrike) from gcRight at the center of the window.
...
class GaitPhaseEstimator(nn.Module):
    def __init__(self, num_channels=12, sequence_length=128, output_dim=1):
        ...
        # Compute the output sequence length after convolution and pooling.
...
if __name__ == "__main__":
    ...
    # --- User parameters ---
    root_directory = "data"  # Update this to your dataset's root directory.
    sequence_length = 128
    batch_size = 8
    ...
    for test_subject in all_subjects:
        ...
        # For demonstration, run only one subject-out split.
        break
